...
d3.queue()
    ...
    .awaitAll(function (error, dataArray) {
        ...
        function createLineChart() {
            ...
            var isDualLine = true;
            var valueline, valueFunc, valueline2, valueFunc2;
            var lineColor, yLabel, lineColor2, yLabel2, dotFuncX, dotFuncY, dotFuncY2;
            ...
            if (isDualLine || (currentMapProp == "varNeonic")) {
                valueline2 = d3.line()
                    .x(function (d) { return x(d.year); })
                    .y(function (d) { return y2(d.totalAllNeonic); });

                y2.domain([0, d3.max(lineData, function (d) { return d.totalAllNeonic; })]);
                lineColor2 = "#079b3b";
                yLabel2 = "Total Neonic Use (lbs)";
                dotFuncY2 = function(d) { return y2(d.totalAllNeonic); }

                valueFunc2 = function(d) { return ctFormat(d.totalAllNeonic); }
            }
y2
            if(isDualLine == false) {
                // We are only showing the first y axis
                y = y2;
                valueLine = valueLine2;
            ...
            */
            ...
            if(isDualLine) {
                myLine2 = svg2.selectAll(".line2")
                .data([lineData]);

                var myLineEnter2 = myLine2.enter().append("path")
                .attr("fill", "none")
                .attr("class", "line2");

                var myLineUpdates2 = myLine2.merge(myLineEnter2);

                myLineUpdates2.transition().duration(1500)
                ...
                // Add dots on line
                var myDots2 = svg2.selectAll(".data-circle2")
                .data(lineData);

                var myDotsEnter2 = myDots2
                    .enter().append("circle")
                    .attr("class", "data-circle2")
                    .on("mouseover", function() {
                        var myEl = d3.select(this);
                        myEl
                            .classed("barHover", true);
                ...
                var myDotUpdates2 = myDots2.merge(myDotsEnter2);
            ...
            if (isDualLine) {
                svg2.selectAll(".yAxisLabel2").remove();

                // now add titles to the axes
                svg2.append("text")
                    .attr("class", "yAxisLabel2")
                    .attr("fill", "#079b3b")
                    .attr("text-anchor", "middle")  // this makes it easy to centre the text as the transform is applied to the anchor
                    .attr("transform", "translate(" + (width) + "," + (height / 2) + ")rotate(-90)")  // text is drawn off the screen top left, move down and out and rotate
                    .text(yLabel2);
